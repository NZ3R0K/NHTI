		...
		#region Draw Shapes
			void DrawRectangle()
			{
				...
				{		
					if (i == rectangle.Verticies.Count - 1)
						DrawLine(new Line(rectangle.Verticies[i] + rectangle.Transform.Position, rectangle.Verticies[0] + rectangle.Transform.Position, rectangle.EdgeColor), true);
					else 
						DrawLine(new Line(rectangle.Verticies[i] + rectangle.Transform.Position, rectangle.Verticies[i + 1] + rectangle.Transform.Position, rectangle.EdgeColor), true);
				}
				...
				for (int i = 0; i < rectangle.Verticies.Count; i++)
				{
					if (i == rectangle.Verticies.Count - 1)
					{
						if (!isRight(thePoint, rectangle.Verticies[i] + rectangle.Transform.Position, rectangle.Verticies[0] + rectangle.Transform.Position))
							return;
					}
					else
					{
						if (!isRight(thePoint, rectangle.Verticies[i] + rectangle.Transform.Position, rectangle.Verticies[i + 1] + rectangle.Transform.Position))
							return;
					}
				}
				...
				if (rectangle.Transform.Position != rectangle.OldCenter)
				{
					rectangle.OldCenter = rectangle.Transform.Position;
					rectangle.updateTransform = true;
				}
				...
				if (rectangle.updateTransform)
				{
					...
					for (int h = 0; h < Screen.height; h++)
					{
						for (int w = 0; w < Screen.width; w++)
						{
							...
							if (isRight(pixel, rectangle.Verticies[0] + rectangle.Transform.Position, rectangle.Verticies[1] + rectangle.Transform.Position) &&
								isRight(pixel, rectangle.Verticies[1] + rectangle.Transform.Position, rectangle.Verticies[2] + rectangle.Transform.Position) &&
								isRight(pixel, rectangle.Verticies[2] + rectangle.Transform.Position, rectangle.Verticies[3] + rectangle.Transform.Position) &&
								isRight(pixel, rectangle.Verticies[3] + rectangle.Transform.Position, rectangle.Verticies[0] + rectangle.Transform.Position))
							{
					...
					rectangle.updateTransform = false;
				}
			...
			void DrawCircle()
			{
				...
				for (int i = 0; i < circle.Sides; i++)
					circle.Verticies.Add(grid.GridToScreen(DrawingTools.EllipseRadiusPoint(circle.Transform.Position, vertexAngle * i, circle.Transform.Scale)));
				...
				if (checkIfPointInsideEllipse(thePoint, circle.Transform.Position, circle.Transform.Scale) >= 1) return;
				#endregion Inside Circle Guard Clause

				#region Refresh Pixel List
				if (circle.Transform.Position != circle.OldCenter)
				{
					circle.OldCenter = circle.Transform.Position;
					circle.updateTransform = true;
				}
				if (circle.Transform.Scale != circle.OldAxis)
				{
					circle.OldAxis = circle.Transform.Scale;
					circle.updateTransform = true;
				}
				...
				if (circle.updateTransform)
				{
					...
					for (int h = 0; h < Screen.height; h++)
					{
						for (int w = 0; w < Screen.width; w++)
						{
							...
							if (checkIfPointInsideEllipse(pixel, circle.Transform.Position, circle.Transform.Scale) < 1)
			...
			void DrawTriangle()
			{
				...
				for (int i = 0; i < triangle.Verticies.Count; i++)
				{
					if (i == triangle.Verticies.Count - 1)
						DrawLine(new Line(triangle.Verticies[i] + triangle.Transform.Position, triangle.Verticies[0] + triangle.Transform.Position, triangle.EdgeColor), true);
					else
						DrawLine(new Line(triangle.Verticies[i] + triangle.Transform.Position, triangle.Verticies[i + 1] + triangle.Transform.Position, triangle.EdgeColor), true);
				}
				...
				if (!isInsideTriangle(
					thePoint, 
					triangle.Verticies[0] + triangle.Transform.Position, 
					triangle.Verticies[1] + triangle.Transform.Position, 
					triangle.Verticies[2] + triangle.Transform.Position)
					) return;
				#endregion Inside Triangle Guard Clause

				#region Refresh Pixel List
				if (triangle.Transform.Position != triangle.OldCenter)
				{
					triangle.OldCenter = triangle.Transform.Position;
					triangle.updateTransform = true;
				}
				...
				if (triangle.updateTransform)
				{
					...
					for (int h = 0; h < Screen.height; h++)
					{
						for (int w = 0; w < Screen.width; w++)
						{
							...
							if (isInsideTriangle(pixel, triangle.Verticies[0] + triangle.Transform.Position, triangle.Verticies[1] + triangle.Transform.Position, triangle.Verticies[2] + triangle.Transform.Position))
					...
					triangle.updateTransform = false;
				}
				#endregion Refresh Pixel List
...
public class CollisionShape 
{
	...
	[HideInInspector] public bool updateTransform = true;
}
